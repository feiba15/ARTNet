...
class Transformer(nn.Module):
 ...
 # 用于动作分类
        self.action_layer = TransformerActionLayer(
            d_model, nhead, dim_feedforward, dropout, activation
        )



        self.noun_layer = TransformerActionLayer(
            d_model, nhead, dim_feedforward, dropout, activation
        )

        self.future_layer = TransformerActionLayer(
            d_model, nhead, dim_feedforward, dropout, activation
        )
    ...
    def forward(
        ...
        if encode_and_save:
            ...
            last_layer_hs = hs[0][-1]
            action_hs, _ = self.action_layer(action_query, last_layer_hs) # 1*(b*t)*F



            noun_hs, _ = self.noun_layer(noun_query, last_layer_hs) # 1*(b*t)*F
            future_hs, _ = self.future_layer(future_query, last_layer_hs) # 1*(b*t)*F
            ...
            if not self.return_weights:
                ...
                return hs.transpose(1, 2), weights, cross_weights,  noun_hs, future_hs, action_hs

class TransformerActionLayer(nn.Module):
    def __init__(
